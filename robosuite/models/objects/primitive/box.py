...
from robosuite.utils.mjcf_utils import get_size, get_size_obstacle #NOTE(dhanush) : Import for the size fucntion for the obstacle
...
class ObstacleObject(PrimitiveObject): #NOTE(dhanush) : We have created a seperate class to control the size sampling, we want it to be discrete
    """
    A box object.
    ...
    def __init__(
        ...
        size = get_size_obstacle(size, size_max, size_min, [0.07, 0.07, 0.07], [0.03, 0.03, 0.03]) #NOTE(dhanush) :  we used the modified function which returns either the max or min size
        super().__init__(
            name=name,
            size=size,
            rgba=rgba,
